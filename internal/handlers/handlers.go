...
// PostShowLogin handles logging that user in
...
// AdminNewReservations show all un processed reservations
...
// AdminAllReservations show all reservations in db
...
// AdminDeleteReservation deletes areservation
...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		// Get the block map from session. Loop through map, if we have an entry in map
		// that does not exist in our posted data, and restriction id > 0 then it is a block
		...
		for name, value := range curMap {
			...
			if val, ok := curMap[name]; ok {
				...
				// the rest are just place holders for days without block
				if val > 0 {
